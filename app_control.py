...
def display_projects():
    create_logger().info("Список проектов")
...
def show_project_details():
    create_logger().info("Детали проекта")
    projects = TaskManager.get_projects()
    try:
        if not projects:
...
def create_task_in_project(project_name):
    create_logger().info("Добавление задачи в проект ")
...
def add_project():
    create_logger().info("Добавить проект")
...
def add_hours_to_employee():
    create_logger().info("Добавить часы сотруднику")
    try:
...
def calculate_employee_pay():
    create_logger().info("Расчет зарплаты")
    employees = EmployeeManager.get_all_employees()
    try:
        if not employees:
            print("\nНет сотрудников в базе данных.")
